...
const TicketFilter = () => {
  ...
  const naver: TicketType[] = useSelector((state) => {
    const returnValue = (state.ticketReducer.tickets ? (state.ticketReducer.tickets as any).array : []) as TicketType[];
  ...
  return (
    <Grid container spacing={3} textAlign="center">
      <Grid item xs={12} md={6} lg={3}>
        <BoxStyled
          onClick={() => dispatch(setVisibilityFilter('naver_webtoon'))}
          sx={{ backgroundColor: 'primary.light', color: 'primary.main' }}
          >
          <Typography variant="h3">{naver.length}</Typography>
          <Typography variant="h6">Naver Webtoon</Typography>
        </BoxStyled>
      </Grid>
      {
      /*
