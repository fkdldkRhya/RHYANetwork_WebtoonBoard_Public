...
from .anaylze_result import AnalyzeResult
...
def event(config_data):
    ...
    questions_webtoon_id = [
        {
            ...
            'name': 'Sub menu',
            'validate': InputIntValueValidator
        }
    ]

    questions_start_article = [
        {
            'type': 'input',
            'message': 'Please enter article start index:',
            'name': 'Sub menu',
            'validate': InputIntValueValidator
        }
    ]

    questions_end_article = [
        {
            'type': 'input',
            'message': 'Please enter article end index:',
            'name': 'Sub menu',
            'validate': InputIntValueValidator
        }
    ]

    questions_comment_target = [
        {
            'type': 'list',
            'message': 'Please select the format of the target comment.',
            'name': 'Sub menu',
            'choices': [
                NEWEST,
                BEST
    ...
    questions_max_type = [
        {
            'type': 'list',
            'message': 'Please select a format for limiting the maximum number of analyses in the target comment.',
            'name': 'Sub menu',
            'choices': [
                ARTICLE,
                ALL
    ...
    questions_max_value = [
        {
            'type': 'input',
            'message': 'Please set a limit on the maximum number of analyses of the target comments:',
            'name': 'Sub menu',
            'validate': InputIntValueValidator
        }
    ]
    ...
    answers_webtoon_id = str(list(prompt(questions_webtoon_id, style=style.get_default_style()).values())[0])
    ...
    answers_start_article = int(list(prompt(questions_start_article, style=style.get_default_style()).values())[0])

    answers_end_article = int(list(prompt(questions_end_article, style=style.get_default_style()).values())[0])

    answers_comment_target = str(list(prompt(questions_comment_target, style=style.get_default_style()).values())[0])

    answers_max_target = str(list(prompt(questions_max_type, style=style.get_default_style()).values())[0])

    answers_max_value = int(list(prompt(questions_max_value, style=style.get_default_style()).values())[0])
    ...
    if answers_max_target == ARTICLE:
        for article_index in range(answers_start_article, answers_end_article + 1):
            ...
            with requests.get(
                    ...
                    f'&type={"NEW" if answers_comment_target == NEWEST else "BEST"}'
                    f'&page={current_page}',
                    headers=get_backend_api_headers(config_data)
            ) as response:
                json = response.json()
    ...
    with requests.get(
            ...
            f'&commentTargetType={"DATE" if answers_comment_target == NEWEST else "BEST"}'
...
class InputIntValueValidator(Validator):
